...
private:
    ...
    //changed
    bool flag_in_memory = false;
    bool flag_is_build = false;
    size_t target_page_num;
    size_t page_num_cnt = 0;
    size_t lru_pointer = 0;
    std::vector<size_t> lru_for_working_memory = std::vector<size_t>(1024, 0);
    std::vector<std::pair<vid_t, size_t>> working_memory = std::vector<std::pair<vid_t, size_t>>(1024); //{src, page_num}
